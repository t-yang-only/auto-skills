...
> 全能旗舰级 AI 研发与多 Agent 协同总控中枢：深度融合多 Agent 牛马协同台账 (nm-skills v2.0)，自洽收拢 44 大顶流核心技能，提供排他防撞、自适应多根寻址与 SDLC 阶段感知流水线。
...
[![Zero Missing Members](https://img.shields.io/badge/Members-44%20Self--Contained-brightgreen.svg)]()
...
- **自洽收拢 44 个高星通用技能**：你只需输入任务意图，调度引擎将自动为你生成端到端的执行流水线，并严格保证各阶段的硬门禁！

---

## 🏗️ 架构与 44 大内置成员 (Architecture & Tools)
...
│   ├── git-guardrails/     # 【防护】拦截 push/reset --hard/clean -f，含放行验证
│   ├── wizard/             # 【向导】带人走只有人能做的人工步骤，段段有判据
│   ├── retro/              # 【复盘】从会话倒推 agent 环境该怎么改；机械性违规建检查，判断题才写标准
│   ├── investigation-first/# 【调查纪律】先出「事实/推断/未知」三栏再下判断；走马观花不算调查
│   └── contradiction-analysis/# 【优先级判断】主要矛盾只能有一个；因果/根源/阶段三检验定主要方面
├── template/
│   └── SKILL.md            # 新技能纳管骨架模板 (--add 缺 SKILL.md 时按此生成)
└── references/
    ├── capability-map.md   # 44 大成员能力全景映射表与冲突优先级
...
| 包 | 必需性 | 缺了会怎样 |
|---|---|---|
| `pyyaml` | **必需** | 配置文件无法解析 → 数据库与私有覆盖层设置全部不生效 |
| `pymysql` | 用数据库落库时必需 | 落库功能不可用（会明确报错并给出安装提示） |

> **为什么单独说明**：`config_manager.py` 早期在缺 `pyyaml` 时是**静默降级**的
> ——`load_config()` 直接跳过配置读取返回默认值，`database.enabled` 变成 `None`，
> 于是落库被判为「已禁用」。使用者看到的是「功能没生效」而不是「少了依赖」，
> 排查方向会被完全带偏。现在缺依赖会显式打印 `[WARN]` 与安装命令。
