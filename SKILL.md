...
description: >
  ...
  Bundles 39 top-tier universal workflow skills with zero-missing multi-root fallback resolution and full SDLC lifecycle stage routing:
...
它深度融合了 **多 Agent 牛马协同台账 (nm-skills v2.0)**，自洽收拢了 **39 个顶流高星通用 Agent Skills**，不仅打通了从「前置自愈 -> 系统认知 -> 需求施压 -> 设计门禁 -> TDD/YAGNI实施 -> 根因调试 -> 5轴代码严审 -> 安全防线 -> 自动化测试 -> 交付收尾与记忆归档」的完整工业级生命周期闭环，更彻底解决了**多 Agent 在同一工程下并发作业时抢跑同一任务、编号同号冲突与代码互撞覆盖**的行业顽疾！
