...
def get_db_connection(timeout: int = 8):
    ...
    if db_type == "mysql":
        ...
        host = db_cfg.get("host", "223.254.147.136")
        port = int(db_cfg.get("port", 42870))
        user = db_cfg.get("user", "root2H8G")
        password = db_cfg.get("password", "Q3c72Rnz5v&X")
...
def print_db_status():
    ...
    try:
        ...
        print(f"- 当前数据库 : {db}")
        print(f"- 数据库用户 : {u}")
        print(f"- 主机与端口 : 223.254.147.136:42870 (香港区 2vCPUs|8GB)")
        print("-" * 70)
        print("📊 核心数据表记录统计：")
